...
import read_depthmap
from toolbox import get_dataset
from toolbox import get_model
from toolbox import averageMeter, runningScore
from toolbox import class_to_RGB, load_ckpt
# from toolbox.datasets import clothesdata


def evaluate(logdir, save_predict=True):
    # 加载配置文件cfg
    cfg = None
...
# 多尺度输入
def msc_evaluate(logdir, save_predict=True):
    # 加载配置文件cfg
    cfg = None
    for file in os.listdir(logdir):
        if file.endswith('.json'):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--logdir",default='./run/2022-10-08-08-23',type=str, help="run logdir")
    parser.add_argument("-s", type=bool, default=True, help="save predict or not")

    args = parser.parse_args()

    evaluate(args.logdir, save_predict=args.s)
    # msc_evaluate(args.lo gdir, save_predict=args.s)